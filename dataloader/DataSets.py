...
class BaseDatasetConfig(object):
    ...
    def __init__(self, args):
        ...
        self.root_dir = str(Path(args.data_root) / self.RelativePath)
...
@Datasets.register("a2n")
class a2n(BaseDatasetConfig):
    Name = 'a2n'
    NumClasses = 23
    SplitRatio = -1
    RelativePath = 'a2n'
    ...
    Classes = {i : k for i, k in enumerate([str(i).zfill(2) for i in range(23)])}

@Datasets.register("a2u")
class a2n(BaseDatasetConfig):
    Name = 'a2u'
    NumClasses = 13
    SplitRatio = -1
    RelativePath = 'a2u'
    Domains = ['aid', 'ucmerced']
    ClassOffset = -1  # text_lists start from 1 not 0
    Classes = {i : k for i, k in enumerate([str(i).zfill(2) for i in range(13)])}

@Datasets.register("u2n")
class u2n(BaseDatasetConfig):
    Name = 'u2n'
    NumClasses = 21
    SplitRatio = -1
    RelativePath = 'u2n'
    Domains = ['ucmerced','nwpu']
    ClassOffset = -1  # text_lists start from 1 not 0
    Classes = {i : k for i, k in enumerate([str(i).zfill(2) for i in range(21)])}

"""
Settings of DomainAdaptor
"""
@Datasets.register("CS")
