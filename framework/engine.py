...
class GenericEngine(object):
    def __init__(self, args, time,tSNE_flag,target_domain_name):
        self.set_seed(time*10000)
        self.args = args
        self.time = time
        self.args.time = time
        self.tSNE_flag = tSNE_flag
        self.args.target_domain_name = target_domain_name
    ...
    def train(self):
        ...
        if self.args.test_with_eval:
            ...
            # test_acc, test_acc_dict = self.test(best_epoch, best_acc, loader=self.source_val,tSNE_flag=self.tSNE_flag,target_domain_name=self.target_domain_name)
            test_acc, test_acc_dict = self.test(best_epoch, best_acc, loader=self.source_val)
        else:
            # test_acc, test_acc_dict = self.test(best_epoch, best_acc, loader=self.target_test,tSNE_flag=self.tSNE_flag,target_domain_name=self.target_domain_name)
    ...
    def test(self, best_epoch=0, best_acc=0, loader=None, tSNE_flag=None, target_domain_name=None):
        lr = self.optimizers[0].param_groups[0]['lr'] if isinstance(self.optimizers, (list, tuple)) else self.optimizers.param_groups[0]['lr']
        self.load_model()
        self.model = self.model.to(self.device)
        # print(tSNE_flag)
        # test_acc, (loss_dict, acc_dict) = EvalFuncs[self.args.eval](self.model, tSNE_flag, target_domain_name, loader, lr, best_epoch, self.args, self, mode='test')
