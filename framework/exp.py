...
class Experiments(object):
    ...
    def run_all_exp_many_times(self, times=3):
        ...
        with open(self.save_path / 'many_exp.txt', 'a') as f:
            ...
            for t in range(times):
                ...
                acc_list, (keys, acc_dict_alist) = self.run_all_exp(self.start_time + t,tSNE_flag)
            ...
            f.write(f'=============Args: a={self.args.joint_a}, b={self.args.joint_b}, c={self.args.joint_c}=============\n')
            ...
            f.write("\n")
            print(f'=============Args: a={self.args.joint_a}, b={self.args.joint_b}, c={self.args.joint_c}=============\n')
            if self.args.do_train:
    ...
    def run_all_exp(self, time, tSNE_flag):
        test_acc_list, test_acc_dict_list = [], []
        with open(str(self.save_path / 'all_exp.txt'), 'a') as f:
            print('------------- New Exp -------------')
            f.write('------------- New Exp -------------\n')
            for i, d in enumerate(self.domains):
                acc, acc_dict = self.run_one_exp(tSNE_flag,d,exp_idx=i, time=time)
    ...
    def run_one_exp(self, tSNE_flag, target_domain_name, exp_idx=0, time=0):
        args = deepcopy(self.args)
        args.exp_num = [exp_idx]
        args.save_path = str(Path(args.save_path) / '{}{}'.format(self.domains[exp_idx], time))
        engine = GenericEngine(args, time, tSNE_flag, target_domain_name)
